...
As opposed to supervised learning, the unsupervised machine learning model is typically not provided truth labels to inform the model of the expected order of the data. 
Instead, the model derives patterns that it finds within the data. 
To do so, unsupervised models often require an underlying assumption of the data, for example a fixed number of groups, dimensions or clusters.
There are three main applications for unsupervised learning; clustering, association rules and dimensionality reduction.


### Clustering

Clustering is an unsupervised form of machine learning in which data is grouped into a provided number of clusters.
This is useful for finding underlying patterns in the data. 
For example, our model has 4 labels for activity: sleep, sedentary, light and MVPA. 
If we run an unsupervised model on this data, specifying that we expect 6 clusters, we may find that the data reveals multiple types of sedentary behaviour (e.g working at desk, lounging at home, eating dinner etc.) and group light and MVPA into one cluster of behaviour.
In this way, unsupervised learning can help support/justify the use of activity labels used.
...
Here we look to discover rules/patterns in our dataset, that can then be informative of the type of data provided.
For example, if we split our data by labels, and provide our association rules model a block of only sleep, we can discover the rules sleep behaviour.
...
This approach of analysis has not been explored througroughly by the OxWearables group, due to the nature of working with free-living datasets in large populations. 
