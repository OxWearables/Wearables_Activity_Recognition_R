...
Copy the required samples into the new directory
```{r copy-samples}
if (!dir.exists(dir_cap24_sample)) {
...
Extract features from each participant file. Note this takes some time.
```{r extract-features}
...
fwrite(dt_feats_all, file = file.path(dir_cap24_sample, "feats_all.csv.gz"), sep = ",")
```

Zip the new folder into a zip file to be uploaded.
Note: this code does not work so I did this manually instead.
```{r zip-samples}
